...
import KeyringController from 'solana-keyring-controller'
import { Mutex } from 'await-semaphore'
import ethUtil from 'ethereumjs-util'
import log from 'loglevel'
import TrezorKeyring from 'eth-trezor-keyring'
import LedgerBridgeKeyring from '@metamask/eth-ledger-bridge-keyring'
import SolanaQuery from 'solana-query'
...
export default class MetamaskController extends EventEmitter {
  ...
  async createNewVaultAndKeychain (password) {
    ...
    try {
      ...
      if (accounts.length > 0) {
        ...
        console.log("createNewVaultAndKeychain - Vault", vault);
        const addresses = await this.keyringController.getAccounts()
        console.log("createNewVaultAndKeychain - Addresses", addresses);
  ...
  async createNewVaultAndRestore (password, seed) {
    ...
    try {
      ...
      const solanaQuery = new SolanaQuery(this.provider)
      accounts = await keyringController.getAccounts()
      lastBalance = await this.getBalance(accounts[accounts.length - 1], solanaQuery)
   ...
   * @param {SolanaQuery} solanaQuery - The SolanaQuery instance to use when asking the network
   */
  getBalance (address, solanaQuery) {
    return new Promise((resolve, reject) => {
      ...
      if (cached && cached.balance) {
        ...
        solanaQuery.getBalance(address, (error, balance) => {
          if (error) {
            reject(error)
            log.error(error)
          } else {
  ...
  */
   ...
   */
  async updateAndSetCustomRpc (rpcUrl, chainId, ticker = 'SOL', nickname, rpcPrefs) {
   ...
   * @returns {Promise<String>} - The RPC Target URL confirmed.
   */
  async setCustomRpc (rpcUrl, chainId, ticker = 'SOL', nickname = '', rpcPrefs = {}) {
