...
export default function BuildQuote ({
  ...
  const fetchParamsFromToken = sourceTokenInfo?.symbol === 'SOL'
    ? swapsEthToken
    : sourceTokenInfo
  ...
  const fromTokenArray = fromToken?.symbol !== 'SOL' && fromToken?.balance
    ? [fromToken]
    : []
  ...
  const selectedFromToken = useTokensToSearch({
    ...
    onlyEth: (fromToken || fetchParamsFromToken)?.symbol === 'SOL',
    singleToken: true,
  })[0]
  ...
  const swapFromFiatValue = fromTokenSymbol === 'SOL'
  ...
  useEffect(() => {
    if (fromToken?.address === ETH_SWAPS_TOKEN_OBJECT.address && (fromToken?.balance !== hexToDecimal(ethBalance))) {
      dispatch(setSwapsFromToken({
        ...
        string: getValueFromWeiHex({ value: ethBalance, numberOfDecimals: 4, toDenomination: 'SOL' }),
      }))
    }
