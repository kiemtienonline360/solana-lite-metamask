...
export function getRenderableTokenData (token, contractExchangeRates, conversionRate, currentCurrency) {
  ...
  const formattedFiat = getTokenFiatAmount(
    symbol === 'SOL' ? 1 : contractExchangeRates[address],
    conversionRate,
    currentCurrency,
    string,
    symbol,
...
export function useTokensToSearch ({
  ...
  return useMemo(() => {
    ...
    memoizedTokensToSearch.forEach((token) => {
      ...
      if (usersTokensAddressMap[token.address] && ((renderableDataToken.symbol === 'SOL') || Number(renderableDataToken.balance ?? 0) !== 0)) {
