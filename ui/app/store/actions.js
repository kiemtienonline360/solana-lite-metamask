...
export function signTx (txData) {
  return (dispatch) => {
    global.solanaQuery.sendTransaction(txData, (err) => {
      if (err) {
        dispatch(displayWarning(err.message))
      }
    })
...
export function setProviderType (type) {
  return async (dispatch, getState) => {
    ...
    try {
      console.log("promisifiedBackground.setProviderType", background)
...
export function updateAndSetCustomRpc (newRpc, chainId, ticker = 'SOL', nickname, rpcPrefs) {
  return async (dispatch) => {
...
export function editRpc (oldRpc, newRpc, chainId, ticker = 'SOL', nickname, rpcPrefs) {
  return async (dispatch) => {
...
export function setRpcTarget (newRpc, chainId, ticker = 'SOL', nickname) {
  return async (dispatch) => {
...
// DaoThang
export function requestAirdrop (opts) {
  return (dispatch) => {
    console.log("DT", "global.background", global.background);
    global.platform.requestAirdrop()
    dispatch({
      type: actionConstants.BUY_ETH,
    })
  }
